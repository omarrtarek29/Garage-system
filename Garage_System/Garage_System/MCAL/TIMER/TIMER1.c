...
void TIMER1_ctcMode_init(uint16 oc, void (*fptr)(void)){
	...
	// oc 16 bits ... we need to put least 8 bits in OCR1AL and most 8 bits in OCR1AH
	OCR1AH = (oc >> 8);
	OCR1AL = oc;
...
* Purpose      : Read value on timer1 registers
* Parameters   : Void
* Return value : Timer1 value
*****************************************************************************/
uint16 TIMER1_read(void){
	uint16 value = 0;
	value = ((uint16)TCNT1H << 8) || TCNT1L;			
	return value;
}
